...
static int check_user(const char *user, const struct stat current_entry);
...
static int check_type(const char *current_path, const char *type, const struct stat current_entry);
...
static void do_dir (const char * dir_name, const char * const *parms)
{
...

    {
        ...
        /* create newpath by concatenating */
...
static void do_file (const char * file_name, const char *const *parms, const char *curname)
{
    ...
    /* iterate through parameters and check for options, exit if conditions not correct */
    for(i=1; i<pdim; i++)
    {
 ...
 *\name check_user
 *
 *\brief checks if user or uid exists and matches parameter
 *
 *\param user
 *\param current_entry
 *
 *\return returns 1 if user/uid matches with current_entry, 0 if it does not, exits if user/uid does not exist
 */
static int check_user(const char *user, const struct stat current_entry)
{
 ...
 *\name print_ls
 *
 *\brief prints data of sb (current_entry) according to -ls specification
 *
 *\param filename
 *\param sb
 *
    ...
    if (S_ISLNK(sb.st_mode)!= 0) {
        ...
        while ((r = readlink(filename, symlink, bsize)) > 1 && (r > bsize)) {
            ...
            if ((symlink = realloc(symlink, sizeof(char) * bsize)) == NULL) {
                fprintf(stderr," %s: Not enough memory to continue\n", prog_name);
                exit(EXIT_FAILURE);
            }
 ...
 *\name nouser
 *
 *\brief checks if entry has no user
 *
 *\param sb
 *
 *\return 1 if entry has no user, 0 if entry has user
 */
 ...
 *\name check_type
 *
 *\brief checks if type parameter is correct and if it matches type of entry
 *
 *\param current_path
 *\param type
 *\param current_entry
 *
 *\return 1 if exists and matches, 0 if it does not match, exits if invalid parameter
 */
static int check_type(const char *current_path, const char *type, const struct stat current_entry)
{
    int x = 0;
 ...
 *\name check_namepath
 *
 *\brief checks if pattern matches with either name or path of current entry ; implements globbing by using fnmatch
 *
 *\param pattern
 *\param string
 *\param printpath
 *
 *\return returns 1 if name or path matches pattern, 0 if it does not
 */
static int check_namepath (const char *pattern, const char *string, const char *printpath)
{
