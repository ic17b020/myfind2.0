...
// Beispiel 1
//
// @author Stammgruppe 14    2B2
// @author Sabrina Mehlmann  ic17b060 <ic17b060@technikum-wien.at>
// @author Gregor Mersich    ic17b020 <ic17b020@technikum-wien.at>
// @author Jan Reichetzeder  ic17b018 <ic17b018@technikum-wien.at>
// @date   2018/04/06
//
// @todo Komplettierung von exec_options
// @todo Fehlerbehandlung für alle Fälle
// @todo Gleichziehen der Parameterbezeichnungen von check-Funktionen
// @todo Kommentare zu allen Funktionen
// @todo Params inklusive Programmname implementieren für Errormessage
// @todo Kontrolle ob Params korrekt ist
...
static void check_print_ls(const char *filename, const struct stat sb);
static void check_user_name(char *current_path, const char *user, const struct stat current_entry);
static void check_uid(char *current_path, int user_id, const struct stat current_entry);
static void check_nouser(const char *filename, const struct stat sb);
static void check_name (char *pattern, char *string, char *printpath);
static void check_type(char *current_path, const char *type, const struct stat current_entry);

//*
// \brief Simple Implementierung von find als myfind
//
// \parameter argc ist die Anzahl der übergebenen Argumente
// \paramter argv sind die Argumente selbst
// \return success
//
...
static void check_user_name(char *current_path, const char *user, const
  const struct stat current_entry)
{
    struct passwd *popt;
    if(getpwnam(user)!=0)
    {
...
static void check_uid(char *current_path, char *userID, const struct stat current_entry)
{
    struct passwd *popt;
...
// Funktion für options ICH BIN NOCH LANGE NICHT FERTIG

static void exec_options(char *current_path, const char **parms, struc stat current_entry)
{
  int i = 0;
  while(parms[i] != 0)
  {
      switch(parms[i])
      {
          case '-user':
          {
            if(parms[i + 1] == "<NAME>") check_user(current_path, parms[i + 1], current_entry);
            else if(parms[i + 1] == "<ZAHL>") check_uid(current_path, parms[i + 1], current_entry);
            else printf("FALSCHE EINGABE");
          } break;
          case '-uid':
          {
            check_uid(current_path, parms[i + 1], current_entry);
          } break;
          case '-name': break;
          case '-type': break;
          //case '-print': break;
